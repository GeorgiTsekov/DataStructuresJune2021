...
namespace P04EightQueensProblem
{
    class Program
    {
        static void Main()
        ...
        private static bool IsSafe(int[,] queens, int row, int col)
        {
            for (int i = 1; i < queens.GetLength(0); i++)
            {
                ...
                if (row + i < queens.GetLength(0) && col + i < queens.GetLength(0) && queens[row + i, col + i] == 1)
                {
                    return false;
                }
                if (row + i < queens.GetLength(0) && col - i >= 0 && queens[row + i, col - i] == 1)
                {
                    return false;
                }
                if (row - i >= 0 && col + i < queens.GetLength(0) && queens[row - i, col + i] == 1)
                {
                    return false;
                }
                if (row - i >= 0 && col - i >= 0 && queens[row - i, col - i] == 1)
                {
                    return false;
                }
            }
